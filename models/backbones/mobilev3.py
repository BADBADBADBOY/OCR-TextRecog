...
def CreateBackboneModel(modelName,pretrained,scale=1,isGray=False):
    assert modelName in ['mobilenet_v3_small', 'mobilenet_v3_large']
    model = globals().get(modelName)(pretrained,scale=scale,is_gray=isGray)
    return model

# model,mapChannels = CreateBackboneModel('mobilenet_v3_large',pretrained=False,scale=1,isGray=True)
