...
if __name__ == '__main__':
    ...
    parser.add_argument('--file', type = str,default='', help = 'path to file which contains the image path and label')
    parser.add_argument('--files', type = list,default=['./error.txt','./error.txt','./error.txt'], help = 'path to file which contains the image path and label')
    ...
    if args.file is not None:
        if len(args.file)!=0:
            ...
            for f in list(args.files):
                _im,_la = read_data_from_file(f)
                image_path_list.extend(_im)
                label_list.extend(_la)
        ...
        with open(os.path.join(args.out,'key.txt'),'w+') as f:
            print(''.join(list(set(''.join(label_list)))))
            print(os.path.join(args.out,'key.txt'))
            f.write(''.join(list(set(''.join(label_list)))))
